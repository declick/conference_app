...
describe("Authentication", () => {
  ...
  describe("Scenario: token is valid", () => {
    it("should return a user", async () => {
      const payload = Buffer.from("johndoe@gmail.com:qwerty").toString(
        "base64"
      );
  ...
  describe("Scenario: email is not valid", () => {
    it("should throw an error", async () => {
      const payload = Buffer.from("unknown@gmail.com:qwerty").toString(
        "base64"
      );
  ...
  describe("Scenario: password is not valid", () => {
    it("should throw an error", async () => {
      const payload = Buffer.from("johndoe@gmail.com:wrong-password").toString(
        "base64"
      );
      await expect(authenticator.authenticate(payload)).rejects.toThrow(
        "Wrong credentials"
